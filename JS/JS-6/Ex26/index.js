...
function createTransactionAmount(amount){
    ...
    const fromatedAmount = formater.format(amount)

    if(amount > 0) {
        span.textContent = `${fromatedAmount} C`
        span.classList.add('transaction-amount','credit')
    } else{
        span.textContent = `${fromatedAmount} D`
...
function createDeleteTransactionBtn(id){
    ...
    //Exluir da tela, do Transactions e do DB
...
//Render te elements into the screen
...
async function saveTransaction(ev){
    ...
    if(id){
        //editar a transação com id existente
